...
async function run() {
    ...
    const disponible = 'Sí'; // Let's try to set to true ('Sí')
    ...
    console.log('Before update:', safeGet(row, 'Disponibilidad'));
    
    try {
        row.set('Disponibilidad', disponible);
        await row.save();
        console.log('Update successful! After update:', safeGet(row, 'Disponibilidad'));
    } catch (err: any) {
