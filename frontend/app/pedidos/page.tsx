          ...
          {order.items.length > 0 ? (
            order.items.map((item, i) => (
              <div key={i} className="flex justify-between items-center py-0.5">
                ...
                {item.price > 0 && <span className="text-[10px] opacity-50">${(item.price * item.quantity).toFixed(2)}</span>}
              </div>
            ))
          ) : (
  ...
  const fetchOrders = async () => {
    ...
    const transformed: Order[] = (data || []).map((o: any) => ({
      ...
      items: o.notes ? o.notes.split(/,|\n/).filter((s: string) => s.trim().length > 0).map((noteStr: string) => {
          ...
          if (match) {
             return { name: match[3].trim(), price: 0, quantity: parseInt(match[1], 10) };
          }
    ...
    // Calculate item prices proportional to total if there's only 1 item type, or just zero it.
    transformed.forEach(t => {
       if (t.items.length === 1 && t.items[0].quantity > 0) {
           t.items[0].price = t.total / t.items[0].quantity;
       }
    });
