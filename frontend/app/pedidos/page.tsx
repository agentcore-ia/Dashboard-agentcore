      ...
      <div className="flex justify-between items-start">
        <div>
          ...
          <div className="flex items-center gap-1.5 text-stone-400 mt-1">
            ...
            <span className="text-xs font-semibold flex items-center gap-2">
              {pickup ? 'Retiro' : 'Delivery'} • {isCall ? 'Llamada' : (isInstagram ? 'Instagram' : (order.customer_phone ? 'WhatsApp' : 'Local'))} • #{order.order_number}
              {!isCall && !isInstagram && order.customer_phone && (
                <a 
        ...
        {order.status === "ready" ? (
          ...
          <div className="flex items-center gap-2">
            {isInstagram && (
              <span className="bg-pink-50 text-pink-600 text-[10px] font-black px-1.5 py-0.5 rounded border border-pink-100">IG</span>
            )}
...
export default function PedidosPage() {
  ...
  const fetchOrders = async () => {
    ...
    const transformed: Order[] = (data || []).map((o: any) => ({
      ...
      customer_name: o.clientes?.name || o.customer_name || (o.source === 'llamada' ? 'Cliente Llamada' : (o.source === 'instagram' ? 'Cliente Instagram' : 'Cliente WhatsApp')),
      ...
      items: o.notes ? o.notes.split(/,|\n/).filter((s: string) => s.trim().length > 0).map((noteStr: string) => {
          // Supports "2 x Napolitanas", "2x Napolitanas", "2 Napolitanas"
          const match = noteStr.trim().match(/^(\d+)\s*(?:x|X|\*|-)?\s+(.*)$/);
          if (match) {
             return { name: match[2].trim(), price: 0, quantity: parseInt(match[1], 10) };
          }
          return { name: noteStr.trim(), price: 0, quantity: 1 };
      }) : [],
