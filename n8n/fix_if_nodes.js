...
async function run() {
  ...
  // Update If nodes 2 to 5 to avoid evaluating empty strings to true
      ...
      wfBody.nodes[idx].parameters = {
        "conditions": {
          ...
          "conditions": [
            {
              "id": nodeName + "c",
              "leftValue": `={{ $('Dividir mensaje').first().json.output.response.${partKey}.length }}`,
              "rightValue": 0,
              "operator": {
                "type": "number",
                "operation": "larger",
  ...
  console.log('Update Status If nodes:', result.status);
}
run();
