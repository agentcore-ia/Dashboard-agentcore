...
async function run() {
  ...
  if (toolIndex !== -1) {
    ...
    let newValues = tool.parameters.parametersBody.values.map(v => {
      if (v.name === 'direccion') return { name: 'direccion', description: 'OBLIGATORIO si es delivery. Debe contener la calle y numero.', type: 'string', required: true };
      if (v.name === 'pedido') return { name: 'pedido', description: 'Detalle de la orden confirmada', type: 'string', required: true };
      if (v.name === 'total') return { name: 'total', description: 'Monto a pagar', type: 'number', required: true };
      if (v.name === 'tipo_entrega') return { name: 'tipo_entrega', description: 'delivery o pickup', type: 'string', required: true };
      return { ...v, required: true };
    });
    
    // Some n8n versions don't natively support putting arrays of objects into specify body 'json', but toolHttpRequest does support descriptions if passed properly in some nodes. Actually, the parameter structure for toolHttpRequest inside N8N uses parametersBody which is an array of pairs. Standard 	oolHttpRequest has 
ame, 	ype maybe.
    // Let's just update the main tool description to be explicitly foolproof and require confirmation.
    
    tool.parameters.description = "ESTA HERRAMIENTA DISPARA LA ORDEN FINAL A LA BASE DE DATOS. REGLAS: 1) LLAMAR UNA SOLA VEZ AL FINAL de la conversacion, cuando el cliente ya confirmo su pedido, su direccion de envio (o aclaro que retira), y como va a pagar. 2) NO LLAMAR PREMATURAMENTE. Si el cliente pide algo pero te falta su direccion o forma de pago, PREGUNTALE PRIMERO y NO uses esta herramienta todavia. 3) Parametro 'direccion': PASAR OBLIGATORIAMENTE un string con la direccion si es delivery (ej: 'Av Falsa 123'), NUNCA DEJAR VACIO a menos que sea Retiro en el local. 4) Nunca llamar dos veces para el mismo pedido.";
