...
async function main() {
  ...
  const mediaNodes = wf.nodes.filter(n => n.name === 'Get Audio' || n.name === 'Get Image');
  
  for (const n of mediaNodes) {
    console.log(`2. Correcting URL for ${n.name}...`);
    n.parameters.url = `=${EVOLUTION_URL}/chat/getBase64FromMediaMessage/${INSTANCE_NAME}`;
    
    // Also fix the apikey in headers
    if (n.parameters.headerParameters && n.parameters.headerParameters.parameters) {
      const apiKeyHeader = n.parameters.headerParameters.parameters.find(p => p.name === 'apikey');
      if (apiKeyHeader) {
  ...
  console.log('3. Pushing media node correction...');
  const result = await apiCall('PUT', `/workflows/${WORKFLOW_ID}`, payload);

  if (result.id) {
    console.log(`   ✅ Media nodes (Audio/Image) fixed with correct Evolution API URL!`);
  } else {
