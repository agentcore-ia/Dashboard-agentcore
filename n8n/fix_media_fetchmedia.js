...
const N8N_API_KEY = "eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJzdWIiOiI2NzA0NzEwYy05NTNlLTQ2MzctODM5My1iN2U5OTZiZTJiN2EiLCJpc3MiOiJuOG4iLCJhdWQiOiJwdWJsaWMtYXBpIiwianRpIjoiYjU5NGEzZDEtOTc5ZC00YzYxLTkouZDEtODdhM2YxOWViODMwIiwiaWF0IjoxNzczNzc5MDc5LCJleHAiOjE3ODE0OTYwMDB9.vBqNhO8OUtF_D5NxIOMsKbPTbKmtutcA-7z64mFzuHA";
...
async function main() {
  ...
  if (getAudioNode) {
    console.log('2. Updating Get Audio to /chat/fetchMedia/...');
    getAudioNode.parameters.url = `=${EVOLUTION_URL}/chat/fetchMedia/${INSTANCE_NAME}`;
    getAudioNode.parameters.responseFormat = "file";
  }

  const getImageNode = wf.nodes.find(n => n.name === 'Get Image');
  if (getImageNode) {
    console.log('3. Updating Get Image to /chat/fetchMedia/...');
    getImageNode.parameters.url = `=${EVOLUTION_URL}/chat/fetchMedia/${INSTANCE_NAME}`;
  ...
  console.log('4. Pushing media fetch fix...');
  const result = await apiCall('PUT', `/workflows/${WORKFLOW_ID}`, payload);

  if (result.id) {
    console.log(`   ✅ Media fetch updated to /chat/fetchMedia/!`);
  } else {
