...
async function main() {
  ...
  for (const info of b.data) {
    ...
    if (rd['Get Audio']) {
      ...
      if (rd['Get Audio'][0].data.main[0]) {
          console.log('Get Audio output:', JSON.stringify(rd['Get Audio'][0].data.main[0], null, 2).substring(0, 500));
      }
      if (rd['Get Audio'][0].error) {
          console.log('Get Audio error:', JSON.stringify(rd['Get Audio'][0].error, null, 2));
      }
      return; 
    }
  }
