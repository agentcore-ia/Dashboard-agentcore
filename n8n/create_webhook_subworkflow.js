...
async function main() {
  ...
  // Replace Orden tool in main workflow with Custom Tool pointing to Webhook
  const mainOrdenIndex = mainWf.nodes.findIndex(n => n.name === 'Orden');
  
  if (mainOrdenIndex >= 0) {
    const position = mainWf.nodes[mainOrdenIndex].position;
    
    // n8n-nodes-langchain.toolHttpRequest doesn't exist natively. N8N uses "HttpRequest" node inside "Call n8n Workflow Tool" or "Custom Tool" + "HTTP Request".
    // Wait! The "API Tool" or "HTTP Request Tool" in N8N Langchain is `n8n-nodes-langchain.toolHttpRequest`. 
    // Yes, 1.48+ has `n8n-nodes-langchain.toolHttpRequest`.
    mainWf.nodes[mainOrdenIndex] = {
      "parameters": {
        "name": "Orden",
        "description": "Guarda el pedido confirmado del cliente. Usar SOLO cuando se tengan todos los datos requeridos. Parámetros requeridos: nombre (del cliente), direccion (de entrega), pedido (detalle de pizzas).",
        "method": "POST",
        "url": "http://localhost:5678/webhook/save-order",
        "sendBody": true,
        "specifyBody": "json",
